                ...
                switch (player.idMark.getIndexMenu()) {
                    ...
                    case ConstNpc.BUY_BACK -> {
                        if (select == 0) { // Người chơi chọn "Đồng ý"
                            Player seller = (Player) PLAYERID_OBJECT.get(player.id);
                            if (seller != null) {
                                BuyBackService.gI().buyCostume(player, seller);
                            }
                        }
                    }
                    ...
                    case ConstNpc.MENU_ADMIN -> {
                        switch (select) {
                           ...
                           //     BossManager.gI().showListBoss(player);
                                     NpcService.gI().createMenuConMeo(player, ConstNpc.He_Thong_Boss, -1, "Hệ thống boss",
                                new String[]{"Gọi Boss " , "List Boss"}
                                 );
                        }
                    }
                        ...
                        switch (select) {
                            case 0 -> {
                                try {
                                    ...
                                    if (clan == null) {
                                        Service.gI().sendThongBao(player, "Bang hội không tồn tại!");
                                        return;
                                    }
                                    ...
                                    // ✅ CHECK LEADER CÓ TỒN TẠI TRONG DATABASE KHÔNG
                                    if (leader == null) {
                                        System.out.println("[DissolveClan] Leader null - Tự động xóa clan ID: " + clan.id + ", Name: " + clan.name);
                                        clan.deleteDB(clan.id);
                                        ClanRankDAO.deleteRank(clan.id);
                                        Manager.CLANS.remove(clan);
                                        Service.gI().sendThongBao(player, "Leader không tồn tại, đã tự động xóa bang hội!");
                                        player.clan = null;
                                    ...
                                    // ✅ KIỂM TRA LEADER CÓ TRONG DATABASE KHÔNG
                                    Player leaderInDB = NTTSqlFetcher.loadById((int) leader.id);
                                    if (leaderInDB == null) {
                                        System.out.println("[DissolveClan] Leader không tồn tại trong DB - Tự động xóa clan ID: " + clan.id + ", Leader ID: " + leader.id);
                                        clan.deleteDB(clan.id);
                                        ClanRankDAO.deleteRank(clan.id);
                                        Manager.CLANS.remove(clan);
                                        Service.gI().sendThongBao(player, "Leader không tồn tại, đã tự động xóa bang hội!");
                                    ...
                                    // ✅ LEADER TỒN TẠI - GIẢI TẢN BÌNH THƯỜNG
                                    clan.deleteDB(clan.id);
                                    ClanRankDAO.deleteRank(clan.id);
                                    Manager.CLANS.remove(clan);
                                    ...
                                    ClanService.gI().sendMyClan(player);
                                    ClanService.gI().sendClanId(player);
                                    Service.gI().sendThongBao(player, "Đã giải tán bang hội thành công!");

                                    System.out.println("[DissolveClan] " + player.name + " giải tán bang: " + clan.name + " ID: " + clan.id);

                                } catch (Exception e) {
                                    e.printStackTrace();
                                    Service.gI().sendThongBao(player, "Lỗi khi giải tán bang hội!");
                                }
                            }
                        }
                    }
                    ...
                    case ConstNpc.He_Thong_Boss -> {
                      switch (select){
                       case 0 -> {
                            this.createOtherMenu(player, ConstNpc.CALL_BOSS,
                                        "Chọn Boss?","Broly","Super_broly","Black_Goku","Super_Black_Goku","KUKU","Map_Dau_Dinh","Rambo"
                            );
                         }  
                        case 1 -> {
                          BossManager.gI().showListBoss(player);
                    ...
                    int[] bossIds = {BossID.BROLY, BossID.SUPER_BROLY, BossID.BLACK_GOKU,BossID.SUPER_BLACK_GOKU,BossID.KUKU};
                    if (select >= 0 && select < bossIds.length){ 
                    BossService.gI().callBoss(player, bossIds[select]);
                    }else{
                        Service.gI().sendThongBao(player, "Không có dữ liệu");
                    }
                }
