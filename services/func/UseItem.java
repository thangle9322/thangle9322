...
public class UseItem {
                ...
                switch (item.template.type) {
                    ...
                    default:
                        switch (item.template.id) {
                            ...
                            case 1778:
                                InventoryService.gI().itemBagToBody(pl, indexBag);
                                PetService.Pet2(pl, 1702, 1703, 1704);
                                Service.gI().point(pl);
                                break;
                            case 1779 :
                                InventoryService.gI().itemBagToBody(pl, indexBag);
                                PetService.Pet2(pl, 1705, 1706, 1707);
                                Service.gI().point(pl);
                            break;
                            case 1780 :
                                InventoryService.gI().itemBagToBody(pl, indexBag);
                                PetService.Pet2(pl, 1708, 1709, 1710);
                                Service.gI().point(pl);
                             break;
                            case 1781 :
                                InventoryService.gI().itemBagToBody(pl, indexBag);
                                PetService.Pet2(pl, 1711, 1712, 1713);
                                Service.gI().point(pl);
                             break;
                            case 1782 :
                                InventoryService.gI().itemBagToBody(pl, indexBag);
                                PetService.Pet2(pl, 1714, 1715, 1716);
                                Service.gI().point(pl);
                             break;
                            case 1783 :
                                InventoryService.gI().itemBagToBody(pl, indexBag);
                                PetService.Pet2(pl, 1717, 1718, 1719);
                                Service.gI().point(pl);
                             break;
   ...
   public void OpenHopThanlinh(Player player, int itemUseiD) {
        if (InventoryService.gI().getCountEmptyBag(player) > 4) {
            ...
            short[] icon = new short[2];
            icon[0] = itemused.template.iconID;
            ...
            icon[1] = aotl.template.iconID;
            icon[1] = wTl.template.iconID;
            icon[1] = gTl.template.iconID;
            icon[1] = jayTl.template.iconID;
            icon[1] = RdTl.template.iconID;
    ...
    private void Pokemon(Player pl, Item item) {
        List<Player> bosses = pl.zone.getBosses();
        boolean checkPokemon = false;
        ...
        if (!checkPokemon) {
            Service.gI().sendThongBao(pl, "Không tìm thấy Pokemon");
            return;
        }
