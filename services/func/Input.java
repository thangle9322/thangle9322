...
public class Input {
                 ...
                 case DISSOLUTION_CLAN:
                    ...
                    if (xacNhan.equalsIgnoreCase("OK")) {
                        try {
                            ...
                            if (!clan.isLeader(player)) {
                                Service.gI().sendThongBao(player, "Chỉ bang chủ mới có thể giải tán!");
                                break;
                            }
                            ...
                            //  CHECK LEADER CÓ TRONG DATABASE KHÔNG
                            if (leader == null) {
                                System.out.println("[DissolveClan] Leader null - Auto delete clan ID: " + clan.id);
                                clan.deleteDB(clan.id);
                                ClanRankDAO.deleteRank(clan.id);
                                Manager.CLANS.remove(clan);
                                Service.gI().sendThongBao(player, "Leader không tồn tại, đã xóa bang hội!");
                                player.clan = null;
                            ...
                            if (leaderInDB == null) {
                                System.out.println("[DissolveClan] Leader không trong DB - Auto delete clan ID: " + clan.id + ", Leader ID: " + leader.id);
                                clan.deleteDB(clan.id);
                                ClanRankDAO.deleteRank(clan.id);
                                Manager.CLANS.remove(clan);
                                Service.gI().sendThongBao(player, "Leader không tồn tại, đã xóa bang hội!");
                    ...
                    try {
                        ...
                        if (number < 0 || number > 99) {
                            Service.gI().sendThongBao(player, "Chỉ được chọn từ 0-99");
                            return;
                        }
    ...
    public void createFormSelectOneNumberLuckyNumber(Player pl, boolean isGem) {
        String text = "";
        if (isGem) {
            text = "Hãy chọn 1 số từ 0 đến 99 giá " + Util.numberFormatTanTai(LuckyNumberCost.costPlayGem) + " ngọc";
        } else {
            text = "Hãy chọn 1 số từ 0 đến 99 giá " + Util.numberFormatTanTai(LuckyNumberCost.costPlayGold) + " vàng";
        }
