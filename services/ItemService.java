  ...
  /*  public boolean isOutOfDateTime(Item item) {
        if (item != null) {
            for (Item.ItemOption io : item.itemOptions) {
                ...
                }else if (io.optionTemplate.id == 1) {
                    int minutePass = (int) TimeUtil.diffDate(new Date(), new Date(item.createTime), TimeUtil.MINUTE);
                    if (minutePass != 0) {
                        io.param -= minutePass;
                        if (io.param <= 0) {
                            return true;
                ...
                }else if (io.optionTemplate.id == 63) {
                    int dayPass = (int) TimeUtil.diffDate(new Date(), new Date(item.createTime), TimeUtil.DAY);
                    if (dayPass != 0) {
                        io.param -= dayPass;
                        if (io.param <= 0) {
                ...
                }else if (io.optionTemplate.id == 64) {
                    int hourPass = (int) TimeUtil.diffDate(new Date(), new Date(item.createTime), TimeUtil.HOUR);
                    if (hourPass != 0) {
                        io.param -= hourPass;
                        if (io.param <= 0) {
                            return true;
                ...
                }else if (io.optionTemplate.id == 65) {
                    int minutePass = (int) TimeUtil.diffDate(new Date(), new Date(item.createTime), TimeUtil.MINUTE);
                    if (minutePass != 0) {
                        io.param -= minutePass;
                        if (io.param <= 0) {
    ...
    if (item != null) {
        for (Item.ItemOption io : item.itemOptions) {
            ...
            switch (io.optionTemplate.id) {
                case 93:
                case 63:
                    passed = (int) TimeUtil.diffDate(new Date(), new Date(item.createTime), TimeUtil.DAY);                    
                    break;
                case 64:
                    passed = (int) TimeUtil.diffDate(new Date(), new Date(item.createTime), TimeUtil.HOUR);                   
                case 65:
                    passed = (int) TimeUtil.diffDate(new Date(), new Date(item.createTime), TimeUtil.MINUTE);
