...
public class SubMenuService {
    ...
    public void controller(Player player, int playerTarget, int menuId) {
        ...
        switch (menuId) {
            ...
            case CUU_SAT:
                ...
                break;
            case BUY_BACK:
                Item costume = BuyBackService.gI().getBuyBack(plTarget);
                if (costume != null) {
                   int cost = ShopService.gI().getItemGemCost(costume.template.id);
                    int priceBuy = (int) (cost * 0.8);
                    int reward = (int) (cost * 0.6);

                    String[] selects = new String[]{"Đồng ý", "Từ chối"};
                    NpcService.gI().createMenuConMeo(player, ConstNpc.BUY_BACK, -1,
                            "Bạn có muốn mua " + costume.template.name + " từ " + plTarget.name + " không?\n"
                            + "Giá cửa hàng: " + cost + " ngọc\n"
                            + "Giá tại đây: " + priceBuy + " ngọc\n"
                            + "Người bán nhận được: " + reward + " ngọc\n"
                            + "(Cải trang của " + plTarget.name + " sẽ không mất)",
                            selects, plTarget);
                } else {
                    Service.gI().sendThongBao(player, "Người chơi này không có cải trang để bán!");
                }
                break;
    ...
    public void showMenuForBuyBack(Player player, Player target) {
        Item costume = BuyBackService.gI().getBuyBack(target);
        if (costume != null) {
            int priceBuy = (int) (costume.template.gem * 0.8);
            showSubMenu(player,
                    new SubMenu(BUY_BACK,
                            "Cải trang " + costume.template.name,
                            priceBuy + " ngọc"));
        } else {
            Service.gI().sendThongBao(player, "Người chơi này không có cải trang để bán!");
        }
