    ...
    public static void showMenu(Npc npc, Player player, boolean isGem) {
        if (!isOpen()) {
            ...
            if (isGem) { // nếu chọn là ngọc thì xử lý ở đây
                LuckyNumberHandler.showMenuCSMM(npc, player,LuckyNumberCost.costGem);
            } else { // nếu chọn là vàng thì xử lý ở đây
                LuckyNumberHandler.showMenuCSMM(npc, player,LuckyNumberCost.costGold);
            }
        }
    }
...
//                LuckyNumberService.spinNumber(player, String.valueOf(RESULT), "Con số may mắn là " + RESULT);
                Service.gI().showYourNumber(player, " ", String.valueOf(RESULT) + "", "Con số may mắn là " + RESULT, 1);
                checkPlayerWin(g.id);
            }
        });
    }
    ...
    public static void checkPlayerWin(long id) {
        ...
        for (LuckNumberData g : players) {
            Player player = NTTSqlFetcher.loadById(g.id);
            if (id == g.id && g.number == RESULT && !g.isReward && player != null) {
                if (g.isGem) {
                    ...
                    finish = "Chúc mừng " + player.name + " đã thắng " + Util.numberToMoney(LuckyNumberCost.costGold) + " vàng với con số may mắn " + RESULT;
