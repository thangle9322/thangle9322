...
public class NPoint {
    ...
    /**
     * Tỉ lệ sức đánh/ sức đánh khi đánh boss
     */
    public List<Integer> tlDameAttBoss;
    ...
    /**
     * Bay hồi phục hp, ki % mỗi giây
     */
    private boolean flyHoiPhucHPKI;
    private long lastTimeFlyHoiPhucHPKI;

    /**
     * Bay hồi phục ki % mỗi giây
     */
    private boolean flyHoiPhucKI;
    private long lastTimeFlyHoiPhucKI;
                ...
                if (io.active == card.Level || (card.Level == -1 && io.active == 0)) {
                    switch (io.id) {
                        ...
                        case 81: //MP+#%/30s
                            ...
                            break;
                        case 85: //Dùng để bay và phục hồi KI
                            this.flyHoiPhucKI = true;
                            break;
                        case 88: //Cộng #% exp khi đánh quái
                            this.tlTNSM.add(io.param);
                            break;
                        case 89: //Dùng để bay và phục hồi HP, KI
                            this.flyHoiPhucHPKI = true;
                            break;
                        case 94: //Giáp #%
...
//                                this.tlNeDon += io.param;
            ...
            for (Item item : this.player.inventory.itemsBag) {
                if (item.isNotNullItem() && item.template.id == 1792) {
                    for (ItemOption io : item.itemOptions) {
                        switch (io.optionTemplate.id) {
                            case 72 ->
                                this.levelBT = io.param;
...
//                                this.tlNeDon += io.param;
        ...
        // thêm các type bông tai khác chỗ này
    }
    ...
    private void setCarot() {
        this.isCarot = false; // reset trạng thái

        try {
            // Kiểm tra inventory và itemsBody
            if (this.player == null || this.player.inventory == null || this.player.inventory.itemsBody == null) {
                return;
            }

            // Chỉ quan tâm item ở vị trí 5
            if (this.player.inventory.itemsBody.size() <= 5) {
                return;
            }

            Item caitrang = this.player.inventory.itemsBody.get(5);
            if (caitrang == null || !caitrang.isNotNullItem()) {
                return;
            }

            // Kiểm tra cải trang id 463
            if (caitrang.template == null || caitrang.template.id != 463) {
                return;
            }

            // Kiểm tra option 115
            if (caitrang.itemOptions != null) {
                for (ItemOption opt : caitrang.itemOptions) {
                    if (opt != null && opt.optionTemplate != null && opt.optionTemplate.id == 115) {
                        this.isCarot = true;
            ...
            Logger.error("setCarot err: " + e.getMessage());
        }
    }

    private void setPhantam() {
        try {
            this.isPhanTam = false;
            if (this.player.inventory != null && this.player.inventory.itemsBody != null && this.player.inventory.itemsBody.size() > 5) {
                Item caitrang = this.player.inventory.itemsBody.get(5);
                if (caitrang != null && caitrang.isNotNullItem()) {
                    if (caitrang.template.id == 458) {
                        this.isPhanTam = true;
                    }
                    if (caitrang.itemOptions != null) {
                        for (ItemOption opt : caitrang.itemOptions) {
                            if (opt != null && opt.optionTemplate != null && opt.optionTemplate.id == 111) {
                                this.isPhanTam = true;
                                break;
                            }
                        }
            ...
            Logger.error("setphantam err: " + e.getMessage());
        }
    }
    ...
    private void setHpMax() {
        ...
        if (this.player.effectSkill != null && this.player.effectSkill.isCarot) {
    ...
    private void resetPoint() {
        ...
        this.isPhanTam = false;
        this.flyHoiPhucHPKI = false;
        this.flyHoiPhucKI = false;
    ...
    public int getDameAttack(boolean isAttackMob) {
        ...
        switch (skillSelect.template.id) {
            ...
            case Skill.MAKANKOSAPPO:
                ...
                if (this.player.setClothes != null && this.player.setClothes.picolo == 5) {
                    dameSkill *= 3 / 2;
                }
                if (this.player != null && this.player.isBoss) {
        ...
        if (this.player.effectSkill != null && this.player.effectSkill.isPhanTam && Util.isTrue(30, 100)) {
            return 0;
        }
    ...
    public void update() {
        if (this.player == null && this.player.effectSkill == null) {
            return;
        }
        // ----- Nạp năng lượng -----
        ...
        // Không có mount -> bay thường trừ KI
        if (player.nPoint.isFly && player.getMount() == -1) {
            int mpSub = this.mpg / (100 * (player.effectSkill.isMonkey ? 2 : 1));
            PlayerService.gI().subMp(player, -mpSub);
            return;
        }

        // Có mount -> lấy item slot 9
        Item mountItem = null;
        if (!player.inventory.itemsBody.isEmpty() && player.inventory.itemsBody.size() < 10) {
            mountItem = player.inventory.itemsBody.get(9);
        }

        // Chỉ xử lý nếu slot 9 thật sự là thú cưỡi
        if (mountItem != null && mountItem.isNotNullItem() && mountItem.template != null && mountItem.template.type == 24) {
            for (ItemOption op : mountItem.itemOptions) {
                if (op == null) {
                    continue;
                }

                if (op.optionTemplate.id == 85) { // hồi KI
                    if (Util.canDoWithTime(this.lastTimeFlyHoiPhucKI, 1000)) {
                        this.lastTimeFlyHoiPhucKI = now;
                        int mpAdd = this.mpMax / 100;
                        PlayerService.gI().hoiPhuc(player, 0, mpAdd);
                    }
                } else if (op.optionTemplate.id == 89) { // hồi HP + KI
                    if (Util.canDoWithTime(this.lastTimeFlyHoiPhucHPKI, 1000)) {
                        this.lastTimeFlyHoiPhucHPKI = now;
                        int hpAdd = this.hpMax / 100;
                        int mpAdd = this.mpMax / 100;
                        PlayerService.gI().hoiPhuc(player, hpAdd, mpAdd);
                    }
                }
            }
        }
