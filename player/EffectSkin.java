...
public class EffectSkin {
   ...
   private void updatePhanTam() {
    try {
        if (this.player.nPoint != null && this.player.nPoint.isPhanTam) {
            if (Util.canDoWithTime(lastTimePhanTam, 10000)) {
                ...
                for (int i = playersMap.size() - 1; i >= 0; i--) {
                    ...
                    if (pl != null && pl.nPoint != null && !this.player.equals(pl) 
                        ...
                        && Util.getDistance(this.player, pl) <= 200) {
                         ...
                         if ( player.nPoint.isPhanTam) {
                            Service.gI().chat(pl, textXinbato[Util.nextInt(0, textXinbato.length - 1)]);                     
                        } else {
                            Service.gI().chat(pl, textXinbato[Util.nextInt(0, textXinbato.length - 1)]);
                        }

                        EffectSkillService.gI().setPhanTam(pl, 5000); 
                    }
                }
